...
from .forms import add_questionForm, add_quizForm
...
def add_quiz(request):
    ...
    if request.method == "POST":
        ...
        messages.error(request, "Quiz could not be added!")
...
def add_question(request, quiz_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(reverse("quiz"))
        messages.error(request, "Question could not be added, try again!")
...
def results(request):
    ...
    if not request.user.is_superuser:
        if Results.objects.all().filter(
            staff=staff.id, quiz_name=actual_quiz.quiz_name
        ):
            quiz_attempts = len(
                Results.objects.all().filter(
                    staff=staff.id, quiz_name=actual_quiz.quiz_name
                )
            )
            quiz_attempts += 1
